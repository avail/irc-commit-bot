...
function handleGitHub(req, res) {
    ...
    switch (req.headers["x-github-event"]) {
        ...
        case "commit_comment":
            ...
            for (var channel of channels) {
                bot.say(channel, util.format("\x02\x0306%s\x03\x02: %s commented on a \x04\x04commit\x04\x04 - %s",
                    repo_full_name,
        ...
        case "issue_comment":
            ...
            if (split_url[split_url.length - 2] == "issues") {
                for (var channel of channels) {
                    ...
                    bot.say(channel, util.format("\x02\x0306%s\x03\x02: %s commented on \x04\x04issue\x04\x04 \"%s\" - %s",
                        repo_full_name,
                        req.body["comment"]["user"]["login"],
                ...
                for (var channel of channels) {
                    ...
                    bot.say(channel, util.format("\x02\x0306%s\x03\x02: %s commented on \x04\x04PR\x04\x04 \"%s\" - %s",
                        repo_full_name,
                        req.body["comment"]["user"]["login"],
                        "\x02\x0303" + req.body["issue"]["title"] + "\x03\x02".replace(/[\r\n]/g, " - ").replace(/[\n]/g, " - "),
                        comment_html_url));
